...
TEST(Broadcaster, HierarchyBroadcasterInitialize)
{
    ...
    ASSERT_TRUE(
        PXR_NS::TfDynamic_cast<HierarchyBroadcasterRefPtr>(broadcaster));
}
...
TEST(Broadcaster, HierarchyBroadcasterTransaction)
{
...

    {
        ...
        PXR_NS::UsdPrim p2 =
            stage->GetPrimAtPath(PXR_NS::SdfPath("/scene/D/a"));
...
TEST(Broadcaster, HierarchyBroadcasterTransactionNested)
{
...

    {
        ...
        stage->RemovePrim(PXR_NS::SdfPath("/scene/sublayerShared/K/k2"));
        {
            ...
            PXR_NS::UsdPrim p =
                stage->GetPrimAtPath(PXR_NS::SdfPath("/scene/test/a"));
            p.SetInstanceable(true);
        }
    }
    ASSERT_EQ(l.GetCount(), 1);
...

    {
...

        {
            unf::NoticeTransaction transaction(broker);
            PXR_NS::UsdPrim p2 =
                stage->GetPrimAtPath(PXR_NS::SdfPath("/scene/A"));
            p2.SetInstanceable(true);
        }
    }
...
TEST(Broadcaster, ChildBroadcasterInitialize)
{
    ...
    ASSERT_TRUE(PXR_NS::TfDynamic_cast<HierarchyBroadcasterRefPtr>(
    ...
    ASSERT_TRUE(
        PXR_NS::TfDynamic_cast<ChildBroadcasterRefPtr>(childbroadcaster));
}
...
TEST(Broadcaster, ChildBroadcasterNoticeSimple)
{
    ...
    ::Test::ChildBroadcasterNoticeListener l =
        ::Test::ChildBroadcasterNoticeListener();
